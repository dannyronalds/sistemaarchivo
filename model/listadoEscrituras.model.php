...
$columns = ['idSol', 'tipEsc', 'codNot', 'otorgante', 'favorecido', 'dia', 'mes', 'anio', 'obs'];
...
if ($num_rows > 0) {
    while ($row = $resultado->fetch_assoc()) {
        ...
        $output['data'] .= '<td>' . $row['tipEsc'] . '</td>';
        $output['data'] .= '<td>' . $row['codNot'] . '</td>';
        $output['data'] .= '<td>' . $row['otorgante'] . '</td>';
        $output['data'] .= '<td>' . $row['favorecido'] . '</td>';
