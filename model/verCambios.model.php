...
$columns = ['numero_solicitud', 'tipo_documento', 'estado_documento', 'fecha_atencion', 'cambio_notario', 'notario_cambiado'];

/* Nombre de la tabla */
$table = "formEstadistica";
...
if ($num_rows > 0) {
    while ($row = $resultado->fetch_assoc()) {
        ...
        $output['data'] .= '<tr>';
        $output['data'] .= '<td>' . $row['numero_solicitud'] . '</td>';
        $output['data'] .= '<td>' . $row['tipo_documento'] . '</td>';
        $output['data'] .= '<td class="' . $color . '">' . $row['estado_documento'] .'</td>';
        $output['data'] .= '<td>' . $row['fecha_atencion'] . '</td>';
        $output['data'] .= '<td>' . $row['cambio_notario'] . '</td>';
        $output['data'] .= '<td>' . $row['notario_cambiado'] . '</td>';
